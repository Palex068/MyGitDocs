Взято [Шпаргалка по консольным командам Git](https://github.com/cyberspacedk/Git-commands)
...
+ [Простейший цикл работ](#простейший-цикл-работ)
+ [Указатели](#указатели)
+ [Настройки](#настройки)
+ [Указание неотслеживаемых файлов](#указание-неотслеживаемых-файлов)
+ [Консоль](#консоль)
+ [Длинный вывод в консоли](#длинный-вывод-в-консоли-vim)
+ [Vim (некоторые команды)](#vim-некоторые-команды)
+ [Консольные команды](#консольные-команды)
    + [Создать новый репозиторий](#создать-новый-репозиторий)
    + [Клонирование репозитория](#клонирование-репозитория)
    + [Просмотр изменений](#просмотр-изменений)
    + [Добавление изменений в индекс](#добавление-изменений-в-индекс)
    + [Удаление изменений из индекса](#удаление-изменений-из-индекса)
    + [Отмена изменений](#отмена-изменений)
    + [Коммиты](#коммиты)
    + [Отмена коммитов и перемещение по истории](#отмена-коммитов-и-перемещение-по-истории)
    + [Временно переключиться на другой коммит](#временно-переключиться-на-другой-коммит)
    + [Переключиться на другой коммит и продолжить работу с него](#переключиться-на-другой-коммит-и-продолжить-работу-с-него)
    + [Восстановление изменений](#восстановление-изменений)
    + [Копирование коммита (перенос коммитов)](#копирование-коммита-перенос-коммитов)
    + [Удаление файла](#удаление-файла)
    + [Перемещение/переименование файлов](#перемещениепереименование-файлов)
    + [История коммитов](#история-коммитов)
    + [Кто написал строку](#кто-написал-строку)
    + [История изменений указателей (веток, HEAD)](#история-изменений-указателей-веток-head)
    + [Ветки](#ветки)
    + [Теги](#теги)
    + [Временное сохранение изменений без коммита](#временное-сохранение-изменений-без-коммита)
    + [Удалённые репозитории](#удалённые-репозитории)
    + [Конфликт слияния](#конфликт-слияния)
    + [«Перенос» ветки](#«перенос»-ветки)
    + [Разное](#разное)
+ [Примеры](#примеры)
    + [Начало работы](#начало-работы)
    + [«Внесение изменений» в корнит](#«внесение-изменений»-в-коммит)
    + [Работа с ветками](#работа-с-ветками)
    + [Работа с ветками, слияние и откат к состоянию до слияния](#работа-с-ветками-слияние-и-откат-к-состоянию-до-слияния)
    + [Работа с ветками, конфликт слияния](#работа-с-ветками-конфликт-слияния)
    + [Синхронизация репозитория-форка с мастер-репозиторием](#синхронизация-репозитория-форка-с-мастер-репозиторием)
    + [Ошибка в работе: закоммитили в мастер, но поняли, что нужно было коммитить в новую ветку](#ошибка-в-работе-закоммитили-в-мастер-но-поняли-что-нужно-было-коммитить-в-новую-ветку)
    + [Нужно вернуть содержимое файла к состоянию, бывшему в каком-либо коммите (известен хеш коммита)](#нужно-вернуть-содержимое-файла-к-состоянию-бывшему-в-каком-либо-коммите-известен-хеш-коммита)
    + [При любом действии с github (или другим удалённым сервисом) запрашивается логин и пароль](#при-любом-действии-с-github-или-другим-удалённым-сервисом-запрашивается-логин-и-пароль)

[Оглавление репозитория](/README.md)
...
+ __Рабочая директория__ — файловая система проекта (те файлы, с которыми вы работаете).
+ __Индекс__ — список отслеживаемых git-ом файлов и директорий, промежуточное хранилище изменений (редактирование, удаление отслеживаемых файлов).
+ __Директория__ `.git/` — все данные контроля версий этого проекта (вся история разработки: коммиты, ветки, теги и пр.).

__Коммит__ — «сохранение» (хранит набор изменений, сделанный в рабочей директории с момента предыдущего коммита). Коммит неизменен, его нельзя отредактировать.
...
[Как использовать консоль Bash в Windows, основные команды](/BashCommands/README.md).  
...
git clone https://github.com/Palex068/MyGitDocs.git    

# клонировать удаленный репозиторий в директорию «FolderName»
git clone https://github.com/Palex068/MyGitDocs.git FolderName 
